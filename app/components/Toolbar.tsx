        ...
        <div className="p-4 flex justify-between items-center border-b border-slate-50 relative">
          ...
          {user && (
            <div className="flex gap-2 text-[.8rem]">
              ...
              <Link
                href="/projects"
                className="flex bg-[#F5F5F5] py-2 px-3 rounded-[5px] items-center gap-2 hover:bg-[#E2E2E2] duration-300"
              >
                <Folder className="w-[1rem]" />
                Proyek
              </Link>
              <Link
                href="/settings"
      ...
      <div className="space-y-2">
        {walls.map((wall: any) => (
          <div
            ...
            className={`flex items-center gap-2 p-2 text-[.5rem] text-[#303030] font-light rounded-[5px] cursor-pointer transition-all border ${
...
const CeilingSettingsManager = memo(
  ({
    activeWall,
    setCeilingPanelWidth,
    setCeilingPanelLength,
    setCeilingPanelDirection,
    setCeilingTraps,
  }: any) => {
    ...
    const model =
      traps.length === 0 ? "FLAT" : traps.length === 1 ? "DROP1" : "DROP2";
    ...
    const handleModelChange = (newModel: string) => {
      if (newModel === "FLAT") {
        ...
        setCeilingTraps(activeWall.id, [
          {
            width: traps[0]?.width || 60,
            dropHeight: traps[0]?.dropHeight || 15,
            gap: 0,
          },
        ]);
    ...
    return (
      <div className="space-y-[1rem] mt-4">
        ...
        <h3 className="font-medium uppercase text-[10px] tracking-widest text-[#303030]">
          Pengaturan: Ruangan
        </h3>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <div className="flex gap-2 bg-gray-50 p-1 rounded border border-gray-200">
                <button
                  onClick={() =>
                    setCeilingPanelDirection(activeWall.id, "horizontal")
                  }
                ...
                <button
                  onClick={() =>
                    setCeilingPanelDirection(activeWall.id, "vertical")
                  }
            ...
            <div>
              ...
              <select
                value={panelLength}
                onChange={(e) =>
                  setCeilingPanelLength(activeWall.id, Number(e.target.value))
                }
                className="w-full h-[32px] border border-gray-200 rounded px-2 text-[10px] bg-white outline-none focus:border-indigo-400"
              >
          ...
          <div className="flex items-center justify-between p-5 border-b border-slate-100">
            <div>
              ...
              <p className="font-light text-[10px] mt-0.5">
                Silahkan Masukan Detail Proyek
              </p>
            </div>
            <button
              onClick={onClose}
...
const CustomerInfoModal = memo(
  ({ isOpen, onClose, customerInfo, setCustomerInfo, onGenerate }: any) => {
    if (!isOpen) return null;
          ...
          <div className="flex items-center justify-between p-5 border-b border-slate-100">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Fill in customer info before generating PDF
              </p>
            </div>
            <button
              onClick={onClose}
...
export default function Toolbar({ wallEditorRef }: { wallEditorRef: any }) {
  ...
  const activeWall = useMemo(
    () => walls.find((w) => w.id === activeWallId) || walls[0],
    [walls, activeWallId],
  );
  ...
  const totalProductCounts = (calcResults?.totalProductCounts || {}) as Record<
    string,
    number
  >;
  ...
  const ceilingPanels = useMemo(() => {
    ...
    walls.forEach((w) => {
      if (w.type === "ceiling" && w.isClosed) {
        ...
        const panelLengthCm =
          w.ceilingPanelLength && w.ceilingPanelLength > 10
  ...
  const totals = useMemo(() => {
    if (!calcResults)
      return { totalArea: 0, totalDesignArea: 0, grandTotalPrice: 0 };
  ...
  const handleSaveProject = async () => {
    ...
    try {
      ...
      walls.forEach((w) => {
        if (w.type === "ceiling" && ceilingPanels[w.id]) {
          ...
          if (p.optimization?.lisDindingSticks > 0) {
            const lisDindingSubtotal =
              p.optimization.lisDindingSticks * lisDindingPrice;
            materialsList.push({
              id: `lisDinding-${w.id}`,
      ...
      const savedData = await saveProjectToDatabase(
        projectName,
        projectData,
        company.id,
        projectId,
  ...
  const handleExport = () => {
    ...
    img.onload = () => {
      ...
      const footerHeight =
        headerHeight + (products.length + 2) * rowHeight + padding * 2;
      ...
      ctx.fillText(
        `Total Wall Area: ${Math.ceil(totals.totalArea)} m²`,
        padding,
        img.height + padding + 55,
      );
      ctx.fillText(
        `Total Design Area: ${Math.ceil(totals.totalDesignArea)} m²`,
        padding,
        img.height + padding + 80,
      );

      let currentY = img.height + padding + 120;
      products
        .filter((p: Product) => p.category?.toLowerCase() !== "plafon")
        .forEach((product: Product) => {
          ...
          if (count > 0) {
            ...
            ctx.fillText(
              countText,
              canvas.width - padding - metrics.width,
              currentY,
            );
            currentY += rowHeight;
          }
    ...
    <div className="w-full md:w-[300px] lg:w-[380px] bg-white h-full flex flex-col border-l border-slate-200 relative z-10 overflow-hidden font-sans">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-24 md:pb-8">
        ...
        <CeilingSettingsManager
          ...
          setCeilingPanelWidth={useCanvasStore(
            (state) => state.setCeilingPanelWidth,
          )}
          setCeilingPanelLength={useCanvasStore(
            (state) => state.setCeilingPanelLength,
          )}
          setCeilingPanelDirection={useCanvasStore(
            (state) => state.setCeilingPanelDirection,
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`flex py-2 px-3 rounded-[5px] items-center gap-2 hover:bg-[#E2E2E2] duration-300 border-[#E5E5E5] border ${listDrawingType === "line" ? "bg-[#E2E2E2] text-[#303030] rounded-[2px] flex text-[.8rem]" : "flex border-[#E5E5E5] py-2 px-3 rounded-[5px] items-center gap-2 hover:bg-[#E2E2E2] duration-300 text-[.8rem]"}`}
              >
                <PenLine className="w-[1rem]" /> Lurus
              </button>
              <button
                onClick={() => setListDrawingType("rectangle")}
                className={`flex py-2 px-3 rounded-[5px] items-center gap-2 hover:bg-[#E2E2E2] duration-300 border-[#E5E5E5] border ${listDrawingType === "line" ? "text-[#303030] rounded-[2px] flex text-[.8rem]" : "bg-[#E2E2E2] flex border-[#E5E5E5] py-2 px-3 rounded-[5px] items-center gap-2 hover:bg-[#E2E2E2] duration-300 text-[.8rem]"}`}
              >
                <Square className="w-[1rem]" /> Kotak
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {isLoadingProducts ? (
              <div className="p-4 text-center text-xs animate-pulse">
                🔄 Loading...
              </div>
            ) : (
              products
                ...
                .map((product: Product) => (
                  ...
                  >
                    <span className="font-bold text-[10px] font-medium">
                      {product.name}
                    </span>
                    <div
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              <Grid2x2 className="w-[1rem]" /> Jendela
            </button>
            <button
        ...
        <div className="flex justify-between items-center gap-4">
          ...
          <input
            ...
            className="w-20 p-2 bg-white border border-slate-200 rounded-lg text-center text-sm font-medium outline-none text-[.8rem]"
          />
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            {isCalculating && (
              <span className="text-[10px] text-indigo-500 font-bold animate-pulse italic">
                Calculating...
              </span>
            )}
          </div>
          ...
          {!isClosed ? (
            <div className="p-8 bg-white rounded-2xl border border-dashed border-slate-300 text-center space-y-2">
              ...
              <p className="text-xs font-medium text-slate-400 leading-relaxed italic">
                Click the wall surface
                <br />
                to begin placing materials
              </p>
            </div>
          ) : (
            <div className="space-y-6 relative">
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-x-4 gap-y-4">
                  {products
                    .filter(
                      (p: Product) => p.category?.toLowerCase() !== "plafon",
                    )
                    .map((product: Product) => {
                      const count = totalProductCounts[product.id] || 0;
                      ...
                      return (
                        <div key={product.id} className="flex flex-col gap-1.5">
                          ...
                          <div className="flex justify-end items-center gap-2 text-[.8rem] text-[#303030]">
                            <span>Subtotal</span>
                            <span className="font-bold">
                              Rp {(count * price).toLocaleString("id-ID")}
                            </span>
                          </div>
                        </div>
                      );
                            ...
                            <>
                              ...
                              <div className="flex justify-end items-center gap-2 text-[.8rem] text-[#303030]">
                                ...
                                <span className="font-bold">
                                  Rp{" "}
                                  {(
                                    lisDindingCount * lisDindingPrice
                ...
                {totals.grandTotalPrice > 0 && (
                  <div className="mt-4 flex items-center justify-between gap-4">
                    <div className="flex flex-col w-[120px]">
                      <span className="text-[.8rem] font-bold text-[#303030]">
                        Total Harga
                      </span>
                      <span className="text-[.8rem] text-[#707070]">
                        Sisa Bahan {wastePercentage}%
                      </span>
                    </div>
                    <div className="flex-1 flex items-center border border-[#E5E5E5] rounded-[5px] p-2 bg-white">
                      <span className="text-[.8rem] font-bold text-[#303030]">
                        Rp {totals.grandTotalPrice.toLocaleString("id-ID")}
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-4">
                  ...
                  {walls.map((wall, wallIdx) => {
                    ...
                    const ceilingData = isCeiling
                      ? ceilingPanels[wall.id]
                      : null;
                    const wallHasContent =
                      Object.values(metrics.productAreas || {}).some(
                        (a: any) => a > 0,
                      ) ||
                      Object.values(metrics.productLengths || {}).some(
                        (l: any) => l > 0,
                      ) ||
                    ...
                    return (
                      <div
                        key={wall.id}
                        className="bg-white rounded-xl border border-[#E5E5E5] flex"
                      >
                          ...
                          {wallHasContent ? (
                            <div className="flex flex-col">
                              {products
                                .filter(
                                  (p: Product) =>
                                    p.category?.toLowerCase() !== "plafon",
                                )
                                .map((product: Product) => {
                                  const area =
                                    metrics.productAreas[product.id] || 0;
                                  const length =
                                    metrics.productLengths[product.id] || 0;
                                  ...
                                  return (
                                    <div
                                      key={product.id}
                                      className="flex justify-between items-center py-4 border-b border-[#E5E5E5] last:border-0"
                                    >
                              ...
                              {ceilingData && ceilingData.optimization && (
                                <>
                                  <div className="flex justify-between items-center py-3 border-t border-[#E5E5E5]">
                                    ...
                                    <div className="text-[#303030] text-[.8rem] font-bold">
                                      {ceilingData.optimization.luasFlatSqM.toFixed(
                                        2,
                                      )}{" "}
                                      m²
                                    </div>
                                  ...
                                  {ceilingData.optimization.luasDropSqM > 0 && (
                                    <div className="flex justify-between items-center py-3 border-t border-[#E5E5E5]">
                                      ...
                                      <div className="text-[#303030] text-[.8rem] font-bold">
                                        {ceilingData.optimization.luasDropSqM.toFixed(
                                          2,
                                        )}{" "}
                                        m²
                                      </div>
                            ...
                            <div className="py-4 text-[.8rem] text-[#A3A3A3]">
                              Tidak ada material
                            </div>
                          )}
                        </div>
                ...
                <div className="absolute top-[80px] left-0 right-0 bottom-0 flex flex-col items-center justify-start pt-16 px-4 bg-white/40 backdrop-blur-[1px] rounded-xl z-20 pointer-events-auto">
                  <div className="bg-white/95 p-6 rounded-xl border border-slate-200 shadow-xl flex flex-col items-center gap-4 text-center max-w-[280px] w-full">
                    ...
                    <p className="text-xs font-bold text-slate-700 leading-relaxed">
                      Silakan login terlebih dahulu untuk melihat hasil
                      perhitungan kebutuhan material dan harga secara detail.
                    </p>
                    <Link
                      href="/login"
          ...
          try {
            ...
            if (stage && walls.length > 0) {
              ...
              for (const wall of walls) {
                ...
                try {
                  wallImages.push(
                    stage.toDataURL({ pixelRatio: 1.5, mimeType: "image/png" }),
                  );
                } catch {
                  wallImages.push("");
                }
